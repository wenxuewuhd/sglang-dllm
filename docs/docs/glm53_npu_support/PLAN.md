...
| 2026-08-28 | **kpool Triton kernel 全量扫描完成**（10 个，实测）：**7 个编译+运行+逐位精确**；4 个挂在 fp8 —— `bishengir-compile` 无法 lower e4m3，**是 A3 的硬件事实不是 triton-ascend 的问题**（把 fp8 换 bf16 后其中 3 个完全正确）；1 个 `_hadamard128` 是 triton-ascend 的 codegen 缺陷（UB 越界，上下文相关） |
