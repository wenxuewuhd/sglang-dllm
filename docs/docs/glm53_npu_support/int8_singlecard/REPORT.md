...
拿本报告实际跑过的形状算（`kpool = 64`，只影响 **extend** 路径；decode 走缓存的 `cu_seqlens_q`）：

| batch | q_len | 最坏需要 | 代码给 | |
|---|---|---|---|---|
| 16 | 13 | 32 | 21 | ❌ 不够 |
| 16 | 3256 | 832 | 831 | ❌ 不够 |
| 2 | 3256 | 104 | 105 | ✅ 够（差 1） |
| 3 | 2731 | 132 | 133 | ✅ 够（差 1） |

**但本轮所有并发测量都是安全的，而且理由是具体的、不是「没崩所以没事」**：
那个「每请求 +1」只在请求的 key 数**跨过 pool 边界**时才兑现。本报告的 extend 批
**全是 ctx=0 的首次 prefill**，13 token 的请求 key 数是 1..13，`//64` 恒为 0，
**每请求恰好 1 个 run**，16 个请求 = 16 ≤ 21。3256 token 那组被 8192 的 chunk 切成
每批 2 条，落在「够（差 1）」那一行。

⚠ **「差 1」不是余量。** 一旦打开 prefix cache（请求带 ctx 进来）、chunked prefill 的续跑、
或者投机解码，那个 +1 就会兑现，界立刻不够。
