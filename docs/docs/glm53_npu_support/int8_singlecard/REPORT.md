...
#### 另有一条不可外推，原因不是 16 专家而是 **TP1** [实测，来自多卡那条线]

❌ **DSA / indexer 相关 kernel 的效率，往多卡外推会过于乐观。**
`index_n_heads = 32`，**TP1 下每 die 拿到全部 32 个头**；TP8 是 4 个，TP16 只剩 2 个。
多卡那条线实测：TP8 → TP16 die 数翻倍，1 M 上 decode 只快 1.29×、TTFT 1.19×，
**因为 1 M prefill 里约 43% 是 kpool 的「选」，而主导项恰好不是被 TP 切得最干净的那部分。**

**所以本报告 §4 / §8.3 里 indexer / DSA 那几行的地板比值，是整个 TP 谱系里最有利的一端。**
这和「同一个 kernel 名在不同 TP 下不是同一个 shape」（§7b.14）是同一个方向的错误，
但这次被切的是**头数**而不是 M/N/K。
...
八条优化之后是 **31.3 ms（−27%）**，见 §6。**
时间的 39% 在 KDA、28% 在 MoE，而 **MoE 读专家权重的那两个 `GroupedMatmul` 已经贴在
带宽地板上（1.03×）** —— 注意这 1.03× 只属于那两个 kernel，不是整个 MoE 家族（家族是 1.56×，
差额几乎全是路由簿记，见 §2）。
起始一步跑 **3578 个 kernel**，现在 **2400 个**；两端 device 时间都与墙钟**逐点吻合**
（起始 42.823 ms profiler vs 42.8 ms 墙钟，现在 31.274 vs 31.3）
...
****八条落地**：五条逐位/逐 token 不变，两条走形状地板判据，一条见 §6.7。** 基线链条一次只动一个变量；每一步都有 profiler 的
...
| **H** = G + §6.7 shared expert 并进 gmm | 31.5~31.9 | 31.5~31.9 | 2444 | −1.2~−1.6 ms |
| **I** = H + §6.8 kpool 两个 arange 每步算一次 | **31.274** | **31.3** | **2400** | **−0.125 ms** |
| | **−27%** | **−27%** | **−1178** | |

⚠ **H 这一行给的是区间，不是我偷懒。** 它是同一个改动、隔着一次服务重启的两次独立测量
（31.541 和 31.935），而**它真正动的那个层族在两次之间只差 0.05 ms**
（MoE 家族 9.618 / 9.670，**−1.44 ± 0.03**）。差额的大头是 `mHC`（3.419 vs 3.614），
**这条改动根本没碰它**。§7b.11 说单次 profile 噪声底是几百微秒 —— 这就是它。
**诚实的写法是「MoE 家族 −1.44 ± 0.03，整步 −1.2~−1.6」，而 kernel 个数 −126 是噪声造不出来的那个信号。**
...
⚠ **本节所有「端到端逐 token 相同」都是在 batch 宽度 1 下测的**（3 条提示各自单独发一次请求），
**而这个条件是必须的，不是随手写的** —— 见 §7b.15：**这个后端上贪心输出不是 batch 不变的**，
不改任何代码、只把 decode 的 batch 宽度从 1 变成 8，输出就会分叉（最早在第 0 个 token）。
**所以「逐位相同」只在固定 batch 宽度下是合法判据。**

**精度**：§6.1、§6.3、§6.6 端到端逐 token 相同（**batch 宽度 1**，3 条提示 × 64 token，3/3）；
§6.2 由 `check_kda` 判定 **18/18 张量逐位相同**（float64 `==`）；
§6.4 / §6.5 非逐位，走 teacher-forced 的形状地板判据。

⚠ **§6.7 是唯一一条非逐位、且本部署的数值判据判不出来的**，见 §6.7 的验证一节 ——
本部署给出 1.01× 地板（无分辨率），**真 checkpoint 上的 GSM8K 由多卡那条线跑，
−0.23pp = 0.35 SE，「没测出问题」**。**别把「没有一条动了数值」这句话套到它头上。**
...
- 并发 1 / 3 / 13 / 16 **四点一致下移**，其中 3 和 13 是非桶值（padded replay）
  ...
  ⚠ **注意这两条的强度不一样**：逐 token 那条**只在 batch 宽度 1 上做过**，
  四个并发那条**只是计时**。
- ✅ **batch > 1 的缺口已经补上，但不是用端到端对拍** —— 那条路被证明构造不出来（§7b.16：
  同一进程、同一宽度、重复跑结果都不一样）。改用
  `layer_check/check_step_metadata.py` **直接驱动 `_step_metadata` 和重算对拍**：
  batch = **1 / 2 / 4 / 13 / 16** 全部 PASS，覆盖命中、原地写失效、原值重写失效、
  **新 forward 复用未改动 buffer**（唯一只有层序抓得住的那格）、以及投机解码下整个旁路。
  **自带负对照**：同一套判据打在去掉两个失效条件的缓存上，五个宽度各抓到 4 处失败。
...
### 6.7 shared expert 并进 routed 的 `GroupedMatmul` [MoE 家族 −1.44 ± 0.03 ms/step]

`glm5_next.py` 原来在非 CUDA 上直接拒绝 shared-expert 融合：
「Shared experts fusion currently requires CUDA devices」。
**那从来不是一条测量结论，只是没人在昇腾上试过。** 门后面的机制早就完整：
`num_fused_shared_experts`、`mlp.shared_experts → mlp.experts.<n_routed>` 的重映射、
`num_experts = n_routed + 1`、`top_k = num_experts_per_tok + 1`。

NPU 上真正缺的是 **top-k**：`fused_topk_npu` 按 `topk_config.top_k` 向厂商算子要专家，
开了融合就会去挑 **9 个 routed 专家**、永远碰不到共享槽。所以补了
`_append_fused_shared_slot`：先要 routed 的 k 个，再追加共享槽，权重取
`1/routed_scaling_factor`，让 `forward_normal` 末尾那次 `*= routed_scaling_factor`
之后共享专家恰好以 1.0 贡献。
...
| | 变化 | µs/step |
|---|---|---|
| `QuantBatchMatmulV3` shared gate_up / down | 42+42 → 0 | −965.8 / −668.9 |
| `SwiGlu` / `ClipByValueV2` / 两个 `DynamicQuant` / `Add` | 42 各 → 0 | **−605.4 合计** |
| `GroupedMatmul` gmm1 / gmm2 | 8 行 16 专家 → 9 行 17 专家 | +297.6 / +142.5 |
| `ConcatD` + `Fill`（追加那一列）+ 9 行的 clamp/finalise | 新增 | +356.3 |
| | **净** | **−1443.7** |

⚠ **我给出的 0.83 ms 上限低了近一倍，因为我只数了那两个 GEMM。**
shared expert 还会 launch `SwiGlu`、`ClipByValueV2`、两个 `DynamicQuant` 和 `Add` ——
**那 605 µs 一起没了。这是「枚举不全」，不是「上下文当普适」，见 §7b.14 的 A/B 两张表。**

⚠ **一条能测不能解释的**：那 24 MiB/层无论如何都要读，1.25 TB/s 下是 845 µs/step。
作为两个独立 `QuantBatchMatmulV3` 它们花 **1634.7 µs（1.93× 那条线）**；
作为 gmm 里的第九组，**边际成本只有 440.1 µs，约那条线的一半**。
**记为观测，不是机制** —— 我无法从字节数解释它（L2 是 168 MB，每层专家工作集 192 → 216 MiB）。

#### 验证：权重先于计时

**① 权重（决定性，通过）** —— 在 `load_weights` 末尾、`process_weights_after_loading`
改写布局之前检查，六项加正负对照全过：

```
slot16 <- shared_experts  w13[:2048]==gate  w13[2048:]==up  w2==down + 三个 weight_scale   OK
slot0  <- experts.0       同样六项                                                          OK  (正对照)
slot16 != slot0                                                                             OK  (负对照)
switch_w13=False, load_up_proj_weight_first=False —— 从 live quant_method 读的，不是猜的
```

⚠ **这一步不是形式。** `deepseek_v2.py:3162` 的 allow-list 注释警告：
compressed-tensors checkpoint 若把 shared expert **loose** 存着，融合路径会**静默装错**。
**GLM 确实是 loose 存的**（288 专家的原版和裁剪版都是，实测读 weight_map）。
**但实测没有装错**，因为 gate/up 的合并就是 routed 专家本来就在走的 `w1`/`w3` 分片加载；
重映射目标是 `n_routed_experts` **算出来的**，注释里那个写死的 256 只是注释。
→ **上游那条判据用「专家数」当代理，对 GLM 是误报。**

**② 幅值（决定性，通过）** —— 扫融合槽位的权重再打分，最小值落在代码推导的值：

```
0.0 → 4.331   0.2 → 0.324   ★0.4 → 0.124★   0.6 → 0.379   1.0 → 1.481
抛物线顶点 0.388，对推导值 1/routed_scaling_factor = 1/2.5 = 0.400
```

**这条不依赖任何地板** —— 它把「数值对不对」换成了一个**有解析答案**的问题。

**③ ⚠ 端到端 logprob（判不出来，不要当成通过）**
teacher-forced `mean|dlp| = 1.243e-01`（3 提示 / 246 个位置；同构型自比恰好 0.0）。
报告 §0 那个 0~2.6e-2 的地板**不适用** —— 那是 BF16 288 专家上 fp32-vs-bf16 的双参考。
在本部署重测地板（同一 baseline 换 bf16 router）得 **1.233e-01**，融合是它的 **1.01×**。

**我不把这写成「通过」。地板本身高到 1.2e-01，说明这个部署上端到端数值判据几乎没有分辨率**
—— 16 专家强制路由，路由极其脆弱。**1.01× 的意思是「测不出来」，不是「刚好合格」。**
**真正让这条改动可信的是 ① 和 ②，它们都是「该精确成立」的判据，不是「该足够小」的。**

**④ 真 checkpoint 上的 GSM8K —— 由多卡那条线在 TP8 / 288 专家上跑**
[实测，`glm53_longctx`，非本部署]

| 轮次 | 构型 | GSM8K | stop rate |
|---|---|---|---|
| 原基线 | INT8 TP8 | 97.42% | — |
| **B′** | 合并树 + 对方全部改动，**关融合** | **97.65%**（1288/1319）| 100.00% |
| **C** | **+ 本条融合**（日志确认 `Shared experts fusion optimization enabled`）| **97.42%**（1285/1319）| 100.00% |

差 3 题 = **−0.23pp**；两轮独立二项之差的 SE 是 `√2 × 0.47 = 0.66pp`，实测 **0.35 个 SE**。

⚠⚠ **这个数排除的是「明显掉精度」，排除不了小于约 1.3pp（2 SE）的真实退化。**
**它是「没测出问题」，不是「证明了没问题」。** 和上面 ③ 那条克制是同一个道理，
只是分辨率高了一个量级（从「判不出任何东西」到「判得出 >1.3pp」）。

⚠ **B′ 那一轮是这次验收里最该做的一件事，而我原本没要。**
我只提了「融合前后对比」—— 但当时树上已经有 **18 个只在 TP1 验过的 commit**，
其中 §6.4 / §6.5 两条是**非逐位**的。不做 B′ 的话，C 一旦掉了会有两个候选原因而**没法挑**。
B′ 单独回答了那 18 个：**288 专家上干净（97.65%，比基线还高 0.23pp）**。
**这条也顺带清掉了 §0 里「16 专家构型下 §6.4/§6.5 未在真 checkpoint 上验」这个悬念。**
**教训：提验收请求时要把「隔离哪个变量」说清楚，不能只说「前后对比」。**

⚠ **未跑：MTP。** `Glm5NextForConditionalGenerationNextN` 继承 DeepSeek 的 gate，
按 `architectures[0]` 拒绝，所以开 MTP 时 **target 融合、draft 不融合**。
各读各 baked-in 的值，自洽，**但没跑过**。

#### 对 CUDA 的影响：无

改的两行在 CUDA 上取值完全不变（`not _is_cuda and not _is_npu` 恰在原 `not _is_cuda` 为假时为假；
SM80 那条本来就只在 `_is_cuda` 为真时才到达 —— 它需要这个保护是因为 `get_device_sm()`
在非 CUDA 上返回 0）。`topk.py` 在 `hardware_backend/npu/` 下，非 NPU 不可达。

#### 也测了也否了

把追加的那一列写成 `torch.nn.functional.pad`：**同样 168 次 launch**（pad 降解成
`MemSet` + `PadV3`）、**3.3× device 时间**、四个并发上都 +1.1 ms/token，输出逐位相同。
**第三次同一形态（§7.2 / §7.7）：换个写法/融合起来 ≠ 更快。**
已把测量写在调用点的注释里，防止有人「简化」回去。

### 6.8 `kpool` 那两个每层重建的 arange [−0.125 ms/step，−22 kernel]

`memory_pool_npu.py` 的 `kpool_decode_update_index_cache`：每个 DSA 层都重建
`torch.arange(batch)` 和 `torch.arange(index_kpool)`。**两个都不依赖层** ——
`index_kpool` 是模型常量，`batch` 一步之内不变。**11 个层各自生成了同一份 `[0,1,2,...]`。**
这是 §6.3 / §6.6 之后**同一形状的第三例**。

| | 构型 H | 构型 I |
|---|---|---|
| `Range` | 36 次/步，226.2 µs | **14 次/步，99.9 µs** |
| kernel 总数 | 2422 | **2400** |
| device | 31.399 ms | **31.274 ms** |
| 墙钟 conc 1 | 31.5~31.9 | **31.3** |

⚠ **−125 µs 低于单次 profile 的噪声底（§7b.11），所以时间本身不足以下结论。**
**扛结论的是 kernel 数**：−22 是确定性的，而且**正好等于从源码数出的 2 × 11 层**；
整步降幅（−125 µs）与被针对的那组自身的降幅（−126 µs）逐一对上，说明**没有别的东西动**。

**精度**：对构型 H **逐位相同**（batch 1 —— 本后端上唯一可做比较的宽度，§7b.16）。

⚠ **我预测残留 16 个，实测 14 个 —— 预测错了。** 我以为「每步仍需各留 1 个」，
**忘了这个缓存跨步也活着**：`batch` 一直是 1、`pool_size` 是常量，warmup 之后再不 miss。
所以省掉的是 22 个而不是 20 个。**归属数对了，残留数错了。**

⚠ **这个缓存不需要指纹，而 §6.6 那个需要**，差别值得写下来：
arange 是 `n` 的**纯函数**，key 只有 `(n, device)`，**不存在一个活张量的内容能让条目过期**；
§6.6 缓存的是从原地写入的 batch 张量推出来的东西，所以必须查 `_version`。
**「能不能只按值做 key」取决于被缓存的东西是不是纯函数，不取决于它看起来多简单。**

⚠ **剩下的 14 个 `Range` 我没能从源码确证归属，不声称可省。**
...
⚠ **附带一条：进程名里的数字不是你以为的那个数字。**
本轮一个 agent 交还机器时报告「die 1 被别人占了，是个 **TP2 BF16** 作业，
多半是你给 die 2 的那个 agent 扩张到了 die 1+2，**值得找他们谈谈**」。
它看到的是 `sglang::scheduler_TP2`。

**那是 rank 2，不是 tp-size 2。** 实际只有一个 `launch_server --tp-size 16`，
`scheduler_TP0..TP15` 十六个齐全 —— **就是我自己刚批准的那次整机窗口。**
按它的推断行动的话，最好的结果是去找一个不存在的进程，
最坏的结果是有人 kill 掉别人 1 M 跑到一半（TTFT 292 s）的某个 rank。

**判据**：数 `scheduler_TP*` 有几个、看 `launch_server` 的 `--tp-size`，
**不要从单个子进程的名字反推作业规模。**
...
**⚠ 但影响比「改个名」大 —— 那个 16 MB 阈值本身就是它反推的**（16 MiB ÷ 1.25 TB/s = **13.42 µs**）。
用图模式实测的 2.8 µs 反推是 3.34 MiB，用本 profile 的 1.4 µs 是 **1.67 MiB**。
**差 9 倍，而且差在「别看了」这个方向上** —— 一条读到「<16 MB 一律 launch 主导、比值没有意义」
的人，会直接放弃去量 2~16 MB 那一段。**这是一条会让人不去测量的指导，比一个错数字严重。**

**做了敏感度扫描**（`--floor-us`，全量在 `data/kernel_roofline_cfgG_floor1.4.txt`）：

| 字节阈值来自 | 带宽受限 | launch 主导 | 高于地板 |
|---|---|---|---|
| 13.5 µs → 16.09 MiB（旧） | 17.363 / 204 | 11.159 / 2219 | **0.287 / 11** |
| 2.8 µs → 3.34 MiB | 17.363 / 204 | 8.647 / 2078 | **3.153 / 163** |
| 1.4 µs → 1.67 MiB | 17.363 / 204 | 8.630 / 2076 | **3.171 / 165** |

**两个结论**：

1. ✅ **头条结论稳**：「带宽受限 17.363 ms / 52.1% / 204 个 kernel，已经贴墙」
   在三个阈值下**一字不变** —— 那些 kernel 搬 128~1210 MiB，离任何合理阈值都远。
   **六条优化的收益也全是实测的，不受影响。**
2. ⚠ **但有约 2.9 ms / 152 个 kernel 被错误地写掉了**：它们从「launch 主导、比值没有意义」
   变成「**可测量地高于自己的地板**」。2.8 和 1.4 之间答案几乎不动，
   **只有 13.5 那个错值是离群的**。

新露出来的是**七组真 GEMM**（`data/kernel_roofline_cfgG_floor1.4.txt`）：

| kernel | n | µs/step | MiB | ×地板 |
|---|---|---|---|---|
| `QuantBatchMatmulV3` shared gate_up | 42 | 979.7 | 16.0 | 1.73× |
| `QuantBatchMatmulV3` shared down | 42 | 698.5 | 8.0 | **2.44×** |
| `BatchMatMulV2` KDA | 34 | 386.2 | 4.0 | **3.36×** |
| `BatchMatMulV2` DSA | 11 | 286.9 | 16.1 | 1.93× |
| `MatMulV2` DSA | 11 | 224.2 | 12.0 | 2.02× |
| `QuantBatchMatmulV3` DSA | 11 | 215.8 | 8.0 | **2.93×** |
| `SparseFlashAttention` | 11 | 354.6 | 4.0 | 9.54×（地板是人工修正过的，见 §4 要点 5） |

前两组是已知的（§8.2 的 shared expert，正在做）。**其余约 1.11 ms 是这轮之前根本没被算过比值的。**

⚠⚠ **「高于地板」不等于「能变快」。** §8.3 的 `HcPre` 正是反例：它是地板的 20 倍，
而**同 shape 的 stock matmul 一样慢** —— 慢的是 shape 不是算子，没有东西可捡。
**这七组的第一步应该照 `HcPre` 那个套路做**：拿 stock 算子在同 shape 上量一遍，
**先分清是「这个算子差」还是「这个 shape 在这台机器上就这样」。** 那是个便宜且决定性的判据。
**在做完这一步之前，这 1.11 ms 是「一片没被量过的地」，不是「一笔待收的钱」。**

#### 判定套路（`tools/hcpre_microbench.py` 的 `sec_refs`，四个对照缺一不可）

对每个可疑 kernel，在**同一个 profile session** 里同时量四样：

| 对照 | 怎么量 | 它排除什么 |
|---|---|---|
| **① 同 shape 的 stock 算子** | `torch.matmul` 同 M/N/K 同 dtype | **最关键**：一样慢 ⇒ 是 shape 不是算子，没得优化 |
| ② 同字节的纯读 | `w.sum()` | 给出这台机器上**可达**的读地板（≠ 理论地板） |
| ③ 同字节的读+写 | `w.clone()` | 分开读带宽和写带宽 |
| ④ 平凡算子 | `torch.add` 单元素 | 给出该 session 的 per-kernel 下限 |

`HcPre` 上跑出来的答案：① 25.6 µs（**和它自己一样**）、② 8.4 µs、④ 1.42 µs
—— 于是「地板的 20×」这个吓人的比值被拆成「shape 差 3×、算子差 0×」，**结论是没得捡**。

⚠ **必须同 session**：per-kernel 下限和可达带宽都跟构型走（§7b.14 的 13.5 / 2.8 / 1.4
就是三个构型下的三个值）。**跨 session 比对照会重演这一整条错误。**

⚠ **跨 TP 度数不能并排比**：这些 kernel 的 M/N/K 里有被 TP 切的维度，
**同一个 kernel 名在 TP1 和 TP8 上不是同一个 shape，比值也不会一样。**
引用时必须带 TP 宽度。
...
#### 归因按次数分配，所以「次数是多个族之和」的组哪个族都归不进去

⚠ **第三种失效方式，本轮由算子用例那条线撞出来** [实测]。
`torch.ops.npu.causal_conv1d` 自己会发两个 `Cast`（profiler 里紧贴在 `causal_conv1d_4` 前面）：
`Cast "2"` 每步 34 次，干净地归给了 KDA；`Cast "1"` **每步 64 次** ——
其中 34 次是 KDA 的 conv 发的，其余属于别处。**64 不匹配任何层族的次数，
于是整组落进了 `unclassified`。**

| | 归因工具报的 | 实际 |
|---|---|---|
| KDA kernel/步 | 272 | **306**（+34） |
| KDA ms/步 | 10.357 | **10.408**（+51.4 µs） |

⚠ **整步的 2400 个 launch 没有错**（那是 `len(rows)/steps`，不经过归属）。
错的是**层族之间的分配**。所以 §8.2 的加总校验查不出这一类 ——
**它校验的是「分项之和 = 总数」，而这里分项之和仍然等于总数，只是分错了族。**
**加总校验能抓「凭空多出来」和「漏掉」，抓不到「记在了隔壁」。**

**和 §7b.13 / §7b.14 是同一个病的第三、第四种形态**：
§7b.13 是一条写死的拆分比例活得比事实久，§7b.14 是一个单点观测被当成常数，这条是
**「调用次数」这个代理在多族共用一个 shape 时失效**。
**两条的共同解药也一样 —— 加总校验。** 除一下就露馅了。

#### ⚠ 但本轮反复出现的其实是**两种**病，别合并成一条

| | 形态 | 例 | 解药 |
|---|---|---|---|
| **A. 上下文相关的事实被当成普适的** | 某个条件下为真 → 当成到处为真 | `13.5 µs`（一次观测当机器常数）；`(256, 384)` 白名单（几个模型当判据）；地板比值（一个 TP 宽度当 kernel 属性）；`_AscendKDAHybrid`（一个类当接口）；`scheduler_TP2`（rank 当 size） | **问「这个数/这个类型在什么条件下成立」，把条件和数字一起写下来** |
| **B. 枚举不全** | 把一个部分当成了全部 | 「shared expert 只有两个 GEMM」→ 上限估低近一倍，漏了 `SwiGlu`/`ClipByValueV2`/两个 `DynamicQuant`/`Add` 共 605 µs；「判据只读权重不读 `weight_scale`」（§7b.8） | **加总校验** —— 分项之和必须等于总数，时间和个数都要对 |

**A 靠提问抓，B 靠算术抓。用错药抓不到。**
（这条区分由长上下文那条线提出。我原本把两者合并成一条「同一个病的第五种形态」，那是错的 ——
「只数了两个 GEMM」不是把上下文当普适，是根本没数全。）

### 7b.15 「逐位相同」有个前提条件，而这个后端不满足它：贪心输出不是 batch 不变的

⚠ **这条推翻的是本报告最常用的那个判据的适用范围** [实测，由长上下文那条线做的控制实验]。

他们给 MTP 设的判据是「贪心 + 精确算术下投机解码是 exact 的，所以输出必须与非投机逐位相同」。
5 条提示里 3 条分叉。**然后他们做了控制：完全不开 MTP，只把 decode 的 batch 宽度从 1 改成 8**：

```
short0   第 16 个 token 分叉
short2   第 0 个 token 就分叉
```

**基线自己跨 batch 宽度就不可复现。**
=> **「输出逐位相同」对任何改变 batch 形状的东西都不是合法判据。**

#### 对本报告的影响：结论不变，但有一个条件一直是隐含的

本报告全部端到端逐 token 对拍**都是 batch 宽度 1**（3 条提示各自单独发请求），
基线也是同样方式采的。**同宽度对同宽度，所以那些 3/3 全等的结论仍然成立。**
但**条件必须写出来**，否则下一个人拿它去跨并发比就会得到假阳性 —— 已补进 §6 和 §6.6。

**不受影响的**：`check_kda` 的 18/18 张量逐位相同（单层 harness，固定形状）、
§6.7 的六项权重 `torch.equal`（**静态张量，跟 batch 无关**）。
**「该精确成立」的判据里，也要分清哪些是静态的、哪些依赖运行时形状。**

#### 一条更强的形式一直躺在 PLAN 里没被写出来

PLAN P6.6c 当初记的是「batch 宽度会挪动 decode 结果，**但不是 padding 的锅**」——
标题在澄清一个误解，**而它的正文其实证明了一个更强的命题**：
**贪心输出在这台机器上不是 batch 不变的。**
更强的那个形式没被单独写下来，于是没人把它当成判据设计的约束。

**一般化**：一条为了排除某个嫌疑而做的实验，结论往往强于它当初要回答的问题。
**排除完之后，回头问一句「我刚才顺带证明了什么」。**

#### 他们换的判据比原来的好，因为它不需要参考

投机解码的**接受长度**本身就是自检信号：verify 若算错，draft 配不上 target 的 argmax，
接受长度会塌向 1.0。实测 1.71–2.00，**两条打满 2.000** ——
**算错的 verify 产不出「每个 draft 都被接受」。**
**这类「系统自己产生的、只有在正确时才会出现的信号」，比任何外部参考都稳**，
因为它不依赖「另一份配置能复现」这个本条刚刚推翻的前提。

### 7b.16 端到端对拍在 batch > 1 上**根本构造不出来**，所以判据必须下沉

§7b.15 说贪心输出跨 batch 宽度会分叉。本轮去补 §6.6 的验证缺口时，**想在「同一个宽度下」
对拍改动前后**，先做了两个控制 —— **两个都不成立** [实测，本部署，13 token 提示，48 token 输出]：

| 控制 | 宽度 1 | 宽度 4 | 宽度 13 |
|---|---|---|---|
| 同一进程内重复 | 相同 | ⚠ **有分歧** | ⚠ **有分歧** |
| 跨服务重启、代码不变 | 相同 | ⚠ 1/4 条不同 | ⚠ 1/13 条不同 |

**注意第一行**：同一个服务进程、同一份代码、同样的提示、同样的宽度，**重复跑结果就不一样**。
（第一个服务上重复两次曾经相同 —— **那是运气，第二个服务上三次就出现了分歧。**
「试了一次没问题」在这种地方等于没试。）

#### 机制：不确定的不是算术，是**批的组成**

宽度 1 在所有对比里都稳定。宽度 1 只有一种可能的组批方式，**而宽度 > 1 没有** ——
客户端能同步的只有「发送」，**组批是调度器的事**，到达时序一变，同一步里凑在一起的请求就变了。

⚠ **我无法从外部区分「模型在固定宽度下不确定」和「我固定不住宽度」。**
但两者对判据的后果一样：**batch > 1 上不存在可复现的基线，所以任何形式的端到端输出对拍
——跨宽度的、同宽度的、同进程的——都不是合法判据。**
这比 §7b.15 强：那条说的是「不能跨宽度比」，这条说的是「**宽度 > 1 就没得比**」。

#### 后果：验证下沉到单元层，而不是放弃

`layer_check/check_step_metadata.py`：**直接驱动 `_step_metadata`，拿它和「清空缓存后重算」对拍。**
在 batch = 1 / 2 / 4 / 13 / 16 上各测四件事 ——
① 一次 forward 的 11 个 DSA 层命中同一份（**对象同一性**，不只是相等）且等于重算值；
② 同一 buffer 写入新值必须失效；③ 原值原地重写也必须失效（保守，`_version` 照样跳）；
④ **新的 forward 复用完全没被碰过的 buffer** —— 指纹看不见，只有层序抓得住，
而这一格的过期命中是**静默的：池长度错、形状对**。外加 ⑤ 开投机解码时缓存必须整个旁路。

**五个宽度全部 PASS。**

⚠ **并且这套判据自带负对照**：同一套测试打在一个「只按 `data_ptr` 做 key、去掉层序检查」的
缓存上——那正是没想过原地写和 buffer 复用的人会写出来的版本——**五个宽度各抓到 4 处失败**。
**一个不会失败的检查等于没检查（§7b.1）**，所以负对照跟着一起跑，不是单独的一次性验证。

#### 一般化

**当端到端判据不可用时，正确的反应是把判据下沉到它还成立的层级，而不是降低标准接受一个弱判据。**
...
#### ⚠ 又一条潜伏的界错误，而本轮的并发数字**碰巧**没踩到

`kpool_indexer_npu.py` 的 `max_visible_pool_runs`（**本线改过的那个文件**，但这个界不是本线写的）：

```python
return -(-n_rows // kpool) + batch + 1      # ceil(n_rows/kpool) + batch + 1
```

**它的 docstring 自己写着正确的推导**：「每请求至多 `ceil(q_len/kpool) + 1` 个 run，
求和至多 `ceil(n_rows/kpool) + batch`」。但
**`sum_b ceil(x_b/k)` 可以远大于 `ceil(sum_b x_b / k)`** —— 前者每个请求各自向上取整。
正确的界是 `sum_b ceil(q_len_b/k) + batch ≤ floor(n_rows/k) + 2·batch`。
**代码少了大约一个 `batch`。**（由长上下文那条线在 MTP verify 批上实测触发：
AI CPU 的 `ScatterElements` errcode `0x2a`，run 序号 scatter 出界。）

⚠⚠ **本节曾经用 `kpool = 64` 算这张表，那是错的：`index_kpool` 是 4**
（`config.json`；64 是 `page_size`）。**而本报告 §1 早就写着「给 n/4 个 pool 打分」——
同一份报告里自相矛盾，我隔着 1300 行没做这个对照。**这是本轮第三次同一形态
（§7b.14 的 13.5 µs、§8.2 的 `HcPre` 常数）：**一个数在一处被写对、在另一处被写错，
而两处从没被放在一起除过。**下面是用 `kpool = 4` 重算的。

拿本报告实际跑过的形状算（只影响 **extend** 路径；decode 走缓存的 `cu_seqlens_q`）。
「实际」是 ctx=0 首次 prefill 下 `pool_lens` 的不同取值个数，「最坏」是 docstring 的
`ceil(q/k)+1` 求和：

| batch | q_len | 实际 run | 最坏 | 代码给 | |
|---|---|---|---|---|---|
| 16 | 13 | **64** | 80 | **69** | ⚠ 实际够（余 5），**最坏不够** |
| 2 | 3256 | 1630 | 1630 | 1631 | ✅ 够 1 个 |
| 3 | 2731 | 2049 | 2052 | 2053 | ✅ 够 4 个 |
| 16 | 3256 | 13040 | 13040 | 13041 | ✅ 够 1 个 |

**本轮所有并发测量仍然是安全的**，但**理由和我原先写的不是同一个**。
原先写的是「13 token 的 key 数 `//64` 恒为 0，所以每请求恰好 1 个 run」——
`//4` 会变 **4** 次，**每请求 4 个 run**，16 条 = 64，对 69 的余量只有 5。
**结论没变，余量比我声称的小得多，而且我给的机制是错的。**

⚠ **「差 1」不是余量，「余 5」也不是。** 一旦打开 prefix cache（请求带 ctx 进来）、
chunked prefill 的续跑、或者投机解码，请求的 key 数就会跨过更多 pool 边界，实际 run 数
向「最坏」那一列靠，而 16×13 那一行的最坏值**已经超了**。
**这和 §7b.8 的 conv state 是同一类：两条都潜伏在「radix cache 关着」这个测量设定后面。**

**修复归长上下文那条线** —— 他们有能复现的负载，我这边的形状构造不出来，
**改了也验不了**。本节记它是因为它决定了本报告的并发数字在什么条件下还成立。

### 8.2 剩下的目标（构型 **H** = 31.399 ms 上重测，并做了加总校验）

**在合并后的树上重新采了一次 profile**（`data/kernel_attribution_cfgH.txt` /
`kernel_roofline_cfgH.txt`）：**31.399 ms/step，2422 次 launch**。
层族和 = 整步（到小数点后三位），kernel 和 2421 对 2422（每族 `len/steps` 的取整差 1）。

⚠⚠ **下表的层族分配偏低，因为归因按调用次数走**（§7b.13 的第三种失效）。
建算子用例时逐个核对了单层真正发的 kernel，找出 **8 组共 133 个 kernel / 303 µs**
被记进了 `unclassified`：KDA 的 `Cast "1"`（34 个 / 51 µs），
以及 DSA 的 `Range` / `DynamicQuant` / `Fill "1;"` / `Cast "1"` / `Cast "1,4096"` /
`IndexCheck "1;1"` / `Fill "2;"`（99 个 / 251 µs）。
**判据**：整网这几组的总次数都容得下 DSA/KDA 那一份，而算子用例复现源码路径时确实发了它们。

| 层族 | 表内 | **修正后** | 占比 |
|---|---|---|---|
| KDA | 10.383 / 272 | **10.408 / 306** | 33.3% |
| MoE | 9.628 / 504 | 9.628 / 504 | 30.8% |
| **DSA** | 5.167 / 891 | **5.394 / 990** | **17.2%** |
| unclassified | 0.968 / 383 | **0.665 / 250** | 2.1% |

**整步 31.274 ms / 2400 launch 不变** —— 移动的是族与族之间的归属。

| 层族 | ms/step | kernel | 对构型 G |
|---|---|---|---|
| KDA | 10.383 | 272 | −0.018 / +0 |
| **MoE** | **9.613** | **504** | **−1.478 / −168** ← §6.7 |
| DSA | 5.167 | 891 | −0.139 / **−22** ← 合并进来的 kpool 改动，**不是本线的** |
| mHC | 3.314 | 180 | −0.277 / +0 ← ⚠ 见下 |
| 未归类 | 1.084 | 405 | −0.008 / +54 |
| lm_head | 0.930 | 1 | +0.003 |
| dense FFN | 0.537 | 54 | −0.031 / −12 |
| 其余 | 0.371 | 114 | |
| **合计** | **31.399** | **2422** | **−1.949 / −148** |

**kernel 数的账是干净的**：本线 §6.7 拿走 126 个（MoE −168、dense −12、未归类 +54），
合并进来的 kpool 改动拿走 22 个，**2570 − 126 − 22 = 2422**。
⚠ **所以本节的 −1.949 ms 不全是 §6.7 的** —— §6.7 自己的净值是 −1.2~−1.6（§6 的 H 行）。

⚠⚠ **而且 5.143 / 5.167 这个 DSA 数字是「上下文约 210 token」的数字。**
采数用的是 13 token 提示（`profile_server_decode.py --prompt-tokens 13`）。
算子用例的序列扫描独立反解出了同一个值：整网 `SparseFlashAttention` 中位 32.08 µs
落在用例曲线的 n=128（28.53）与 n=256（34.43）之间。
**同一个 DSA 家族在 4 k 以上是 5.62~5.81 ms（+10~13%）** ——
`SparseFlashAttention` 在 n≈4096 被 `index_topk=2048` **彻底封顶**（101.19 → 101.13 → 102.04），
之后唯一还在涨的是 `LightningIndexer`（15.1 → 32.2 µs）。**引用 DSA 的绝对值必须带上下文长度。**

⚠ **`mHC` 掉了 0.277 ms，而本线没有任何改动碰它。**
`HcPre` 还是 90 次/步，但单次从 **33.0 µs 变成 29.9 µs**。
**同一个 kernel、同一个 shape、跨服务重启差 10%** —— §8.3 里那些 `HcPre` 的绝对数
（33.0 µs / 2.969 ms）**应该读成「约 30~33 µs / 约 2.7~3.0 ms」**。
这一条是本轮第二次撞上「我以为是常数的东西其实是个区间」（第一次是 §7b.14 的 13.5 µs）。

**「簿记」在这里有一个明确的定义**（上一版没写下来，见 §7b.13 的教训）：
**一个层族里非 cube 类（不是 `AI_CORE` / `MIX_AIC`）、且不是该层类型专用厂商算子的部分。**
按这个定义重算（构型 H），**四项之和 6.915 ms = 整步的 22.0%**（shared expert 那项已被 §6.7 做掉），未超 100%，无重复计数；
层族分解配平到小数点后三位
...
| | ms/step | 构型 F | 判断 |
|---|---|---|---|
| **`HcPre`** | **2.693** | 2.969 | ⚠ **现在最大的单项**（⚠ 跨重启在 2.69~2.97 之间摆，见上），但**不是「算子写得差」** —— 33.0 µs 里约 **25.6 µs 是一个 `(1,16384)×(16384,24)` fp32 GEMV**，而同 shape 的 stock `aclnnMatmul` **一样是 25–26 µs**。**慢的是这个 shape，不是这个算子**。已刻画完毕并给出厂商问法，见 **§8.3**。**本线改不动** |
| **DSA 的簿记** | **2.164** | 3.252 | §6.6 砍掉 966 µs（每层 84 → 70 个非 cube kernel）。**剩下的大部分是每层必须做的活**：ring 槽维护、选中 token 的散读散写。**按目标分工归 glm53_graph_perf**（P6.7 / P6.10 / P6.11） |
| **MoE 的路由簿记** | **1.876** | 2.171 | ⚠ **拆开之后大部分动不了**：`MoeInitRoutingV3` 594 + `DequantSwigluQuant` 255 + `MoeFinalizeRoutingV2` 247 + `MoeGatingTopK` 188 + `Cast[1,8]` 59 = **1343 µs（62%）是本质的厂商算子**；routed 预 clamp 278 **没有融合路径**（§7.2）；shared clamp+swiglu 282 **融了反而更慢**（§7.7）；`Add`+`Muls` 160 **低于噪声**（§7.3）。**真正可动的约 108 µs** |
| ~~**shared expert 两个量化 GEMM**~~ | ~~1.678~~ | ~~1.666~~ | ✅ **已做掉，§6.7**。⚠ 实际拿到 **1.44 ms**，而不是这里估的「约 0.8 ms 可动」—— 这一格只数了两个 GEMM，漏了同一路径上 605 µs 的向量 kernel（§7b.14 的 B 类错误） |
| KDA 的簿记 | **0.182** | 0.201 | §6.5 之后基本清空（曾是 3.383）—— 那 3.2 ms 几乎全是被拆开的卷积 |
...
构型 H（`tools/kernel_roofline.py`，全量在 `data/kernel_roofline_cfgH.txt`）：

| | ms/step | 占比 | kernel/step | 起点（构型 A） | 能不能动 |
|---|---|---|---|---|---|
| 带宽受限（≥16 MB） | **17.799** | **56.7%** | 204 | 20.022 / 215 | ❌ 已贴墙（MoE gmm 0.99× 地板、KDA qkv 1.07×、lm_head 0.91×） |
| **launch 主导（<16 MB）** | **8.992** | **28.6%** | **2071** | 16.662 / 3163 | ✅ **空间一直在这里** |
| compute / 固定成本 | 4.308 | 13.7% | 136 | 4.899 / 147 | 部分（`HcPre` 约 2.7 在其中，占了大半） |
| 高于地板（≥16 MB） | 0.301 | 1.0% | 11 | 1.241 / 53 | §6.7 把 shared expert 那两个 GEMM 从这里搬走了 |

⚠ **带宽那一栏从 17.363 涨到 17.799，这不是退步。** §6.7 把 shared expert 的
24 MiB/层从两个独立 GEMM（原本记在「高于地板」，1.93× 那条线）搬进了 `GroupedMatmul`
的第九组 —— **字节没变，只是从一个「离地板远」的桶换到了「贴着地板」的桶**。
所以「高于地板」从 1.267 掉到 0.301，占比上升只是因为分母小了。
**读占比之前先看绝对值和分母。**

**七条优化几乎全打在 launch 主导那一栏：16.662 → 8.992 ms、3163 → 2071 个 kernel，
拿走了整步 11.424 ms 里的 7.670 ms。**
...
⚠ 构型 H 上这一栏回升到 17.799，原因见上（是搬进来的字节，不是变慢）。
**没有一条优化让某个 kernel 算得更快 —— 拿到的全是「少发」和「不再互相拖慢」。**

⚠ **这张表的第四行「高于地板」以前是 1.267 ms，那是用错阈值算的**；按实测的 per-kernel
地板重算是 **3.171 ms / 165 个 kernel**，其中约 1.11 ms 是这轮之前从没被算过比值的
七组 GEMM 里的五组（§7b.14）。**它们是「没被量过的地」，不是「待收的钱」** ——
第一步是拿 stock 算子同 shape 对照，分清是算子差还是 shape 差（§8.3 的 `HcPre` 就是后者）。
