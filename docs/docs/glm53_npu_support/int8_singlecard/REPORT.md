...
| | ms/step | 构型 F | 判断 |
|---|---|---|---|
| **`HcPre`** | **2.969** | 2.969 | 33.0 µs 搬 1.5 MiB = 地板的 25×。**已证明不是 sinkhorn**（§7.5）—— 是算子自身的固定开销，**本线改不动，该问厂商**。⚠ 这是现在最大的单项 |
...
**带宽那 52.1% 现在是硬地板。** 要继续下去只有两条路：
换更省字节的权重表示（KDA 投影仍是 bf16，见 §3 / §7.1），或者把活搬走。
