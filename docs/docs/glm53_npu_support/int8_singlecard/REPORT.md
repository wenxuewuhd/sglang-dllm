...
**全默认参数 max|Δ| = 156**（就是 §2.4 那个 109× 陷阱），**对「完全不 clamp」max|Δ| = 2.9e4**。
...
### 7b.8 「逐位相同」有一个明确的盲区：写坏了一块暂时没人读的内存
...
| **MoE 的路由簿记** | **2.171** | ⚠ **拆开之后大部分动不了**：`MoeInitRoutingV3` 572 + `DequantSwigluQuant` 258 + `MoeFinalizeRoutingV2` 242 + `MoeGatingTopK` 191 + `Cast[1,8]` 58 = **1321 µs（61%）是本质的厂商算子**；routed 预 clamp 295 **没有融合路径**（§7.2）；shared clamp+swiglu 277 **融了反而更慢**（§7.7）；`Add`+`Muls` 169 **低于噪声**（§7.3）。**真正可动的约 108 µs** |
