...
## ⚠ 用图的桶大小做并发扫描 = 永远测不到 padded replay

`glm53_int8_1card` 2026-08-30 踩到并提醒的，**本线同样中招**。

图捕获的 bs 桶是 `[1,2,4,8,12,16,24,...]`。**并发数正好落在桶上时，一行 padding 都没有** ——
而 padding 行才是图模式独有的失败模式（每个 padding 行都自称是请求 0，
`req_pool_indices` 被 clamp，`cache_indices` 被置 `-1`）。

本线此前的扫描用的是 **1 / 8 / 16 / 32 / 64 / 128 —— 全是桶大小**。
`tools/check_graph_padding.py` 是专门为此写的（用 3/5/7/13），但**性能扫描没跟上**。

**规则：任何做 A/B 或性能扫描的地方，并发数里必须混入非桶值。**
建议固定用 **1 / 3 / 13 / 16**：一个无 padding 的基准、两个有 padding 的、一个满桶。


---

## 非逐位相同的改动，必须用 teacher forcing 量，不能用自由生成

一个改动如果不是逐位相同（比如把 4 个矩阵乘融合成 1 个 —— GEMM 形状变了，
而 bf16 matmul 不是 batch-shape 不变的），判据就得是**形状地板**而不是逐位。
但**怎么量**决定了你会不会把一条好改动判死。

`glm53_int8_1card` 2026-08-30 在同一个改动上两种量法的结果：

| 提示 | 自由生成（错）| teacher forcing（对）|
|---|---|---|
| 0 | 4.622e-03 | 8.546e-04 |
| 1 | **2.849e-01** ← 看着超标 11× | **1.911e-02** ✅ |
| 2 | 1.724e-02 | 4.192e-03 |

**差了 15 倍，全部来自测量方法。** 那个 2.849e-01 不是数值误差，是**两段不同的续写**：
贪心路径一旦分叉，后面比的就是**不同 token 的 logprob**，那个数字没有意义。

**规则**：
- **prefill 侧**用 teacher-forced logprob（`logit_check.py compare` 本来就是）
- **decode 侧**只在**分叉之前的公共前缀**上比。⚠ 本工具此前**没有做到** ——
  `_compare_decode` 算 `max|dlp|` 用的是全长而不是公共前缀，
  也就是说它在序列分叉后会报出同样无意义的大数字。2026-08-30 已修
- 序列分叉本身是**离散信号**（报"第几个 token 开始分叉"），不要把它和连续的 logprob 差混在一个数字里


---

## 验收只覆盖"被读的东西"，看不见"被写的东西"

2026-08-30。`check_kda` 6/6 全绿、kpool 的 logprob 对拍 `0.000e+00`，而
`causal_conv1d_fn_npu` 正在写坏 conv state。两套信号都亮着，bug 在下面。

**两个洞是同一个形状**：
- `check_kda.states()` 只读 `golden_slot` 一个槽 —— 批里其余 N−1 个槽的 state
  被写了，从不比对
- `logit_check` 比的是 logprob —— **逐位相同排除不了"写坏了一块暂时没人读的内存"**

**而且 harness 到不了触发形状。** `prefill()` 里 `seen = [0]*batch` 齐步走，
所以每个 chunk 内 `has_initial_state` 是齐的（第一个 chunk 全 False，之后全 True）。
`_decoy_len` 只让**长度**参差。**"一批里既有 prefix-cache 命中又有冷请求"
——生产上最普通的流量——在这个 harness 里构造不出来。**

**补法**：`check_kda.py --check-mixed-state WARM`。前 WARM 个槽先预填（拿到真实非零
state），再走一次混合 extend，然后**每个槽**的 state 对"单独重放这一个请求"比对 ——
**一个请求的 state 只能取决于它自己的 token**。实测（layer 0 / rank 0 / batch 8）：

| | conv err | ssm err |
|---|---|---|
| 热槽（`has_initial_state=True`）| **0**（一个槽 2.4e-07）| 在地板内 |
| 冷槽（`False`）| **0.87 – 1.55**，而 `\|conv\|` ≈ 1.0–1.3 | **在地板内** |

坏的**只有 conv state，SSM 没事**；把冷热分界从 4 挪到 6，失败集合跟着从 [4-7] 变成
[6-7]，**所以是 `has_initial_state` 这个属性，不是槽号**；且没有任何一个被写坏的
state 等于别的槽的 state，**排除了索引串号**。

⚠ **地板要汇集，不能按槽单采样。** solo 重放是不同的 batch 形状，bf16 归约顺序不同，
SSM 差在什么都没错时就在 **1.1e-05 ~ 5.8e-05** 之间晃。第一版按槽用自己那一次
uniform 采样当地板，把一个干净的槽判成污染（它是自己那次采样的 2.4 倍，
却远低于另一个槽的采样）。**一次采样不是地板** —— 这条和本文档开头
"判据是测出来的地板，不是拍的阈值"是同一条，只是这次踩在"测了但只测了一次"上。
...
2026-08-30，`glm53_int8_1card` 对自己的一处判断做的修正，**和 P6.7 那个重复计数是同一个病**。

它把 MoE 路由簿记整块 **2.171 ms** 标成了「目标」，理由是「地板是 0」。拆开之后：

```
MoeInitRoutingV3 572 + DequantSwigluQuant 258 + MoeFinalizeRoutingV2 242
+ MoeGatingTopK 191 + Cast 58   = 1321 µs（61%）本质的厂商算子，动不了
routed 预 clamp 295             没有融合路径（那个融合算子是假的）
shared clamp+swiglu 277         可融合，约值 108 µs
Add + Muls 169                  已测低于噪声
```

**真正可动的只有约 108 µs。** 一次路由置换本来就得发生 —— 它的带宽地板是 0，
但它的**工作量不是 0**。

**和 P6.7 是同一个病**：P6.7 是同一笔账从「函数」和「阶段」两个角度各量了一次，
于是在待办里变成两条独立条目；这条是把一个**测量值**当成了**可回收量**。
**两次都是没问「这个数字代表的工作是不是必须做」。**

⚠ 由此推出一条通用的：**「X 占了 N 毫秒」不是待办，「X 里有 M 毫秒不必发生」才是待办。**
前者是观测，后者才是判断，而从前者到后者需要一次拆解 —— 跳过这一步，
待办清单里就会混进一批做不动的东西，而且它们看起来最大。
