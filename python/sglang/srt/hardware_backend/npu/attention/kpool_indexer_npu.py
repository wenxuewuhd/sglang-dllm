...
class KPoolNPUIndexerMixin:
    ...
    def _compress_write_plan(extend_seq_lens, seq_lens, n_rows: int, kpool: int):
        ...
        * ``tail_rows`` / ``tail_valid`` / ``tail_slots_logical`` ``[B, kpool-1]``
        ...
        tail_rows = ((row_starts + n_pools_per * kpool).unsqueeze(1) + t).clamp(
            max=last_row
        )
    ...
    def forward_npu(
        ...
        pool_lens_row = (
            ...
            else torch.div(seq_lens_row, self.index_kpool, rounding_mode="floor").to(
                torch.int32
            )
        )
        if mode.is_decode_or_idle():
