...
def joint_threshold_update_step_vectorized(
    ...
    if penalty_lambda > 0:
        ...
        # this branch keeps the fp32-copy path: the copy is what the scatter
        # may mutate (never the graph's lm_head-dtype logits buffer).
...
class JointThreshold(DllmAlgorithm):
    ...
    def _step_per_row(
        ...
        for i in range(batch_size):
            ...
            # Row-sized fp32 copy: keeps this path's original numerics when the
            # incoming logits stay in lm_head dtype, and keeps the penalty
            # scatter off the graph's logits buffer.
