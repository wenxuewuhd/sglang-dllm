...
class SchedulerDllmMixin:
    def init_diffusion_llm(self: Scheduler):
        ...
        self.dllm_mixed_batch_enabled = (
            self.dllm_config is not None and envs.SGLANG_ENABLE_DLLM_MIXED_BATCH.get()
        )
    ...
    def _process_dllm_batches_mixed(
        ...
        staging_reqs = [
            ...
            for req in reqs
            if req.dllm_phase
            in (DllmReqPhase.STAGING_DECODE, DllmReqPhase.STAGING_PREFILL)
        ]
        if staging_reqs:
        ...
        incoming_reqs = [
            ...
            for req in reqs
            if req.dllm_phase
            in (DllmReqPhase.INCOMING_DECODE, DllmReqPhase.INCOMING_PREFILL)
        ]
        if incoming_reqs:
    ...
    def _process_dllm_incoming_reqs_mixed(
        ...
        the free pool. Gate instead on rows that still need a fresh slot vs
        the free pool, plus the total-row cap. Priority preemption is not
        wired for mixed rounds.
        """
        running_bs = len(running_batch.reqs)
