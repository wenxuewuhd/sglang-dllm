...
# ---------------------------------------------------------------------------
# Vendored from huggingface/transformers, src/transformers/models/glm5_next/image_processing_glm5_next.py,
...
#   * `get_number_of_image_patches` applies `patch_expand_factor`, matching the
#     resize factor `_preprocess` uses; upstream omits it and the two disagree
#     whenever the factor is not 1
# ---------------------------------------------------------------------------
...
class Glm5NextImageProcessor(TorchvisionBackend):
    ...
    def get_number_of_image_patches(
        ...
        # LOCAL FIX (not upstream): `_preprocess` resizes on
        # `patch_size * merge_size * patch_expand_factor`, so the factor here must
        # match or the token count this returns describes a different resize than
        # the one that produced the pixels. Upstream drops patch_expand_factor and
        # the two agree only while it is 1 (which is what the GLM-5.3-Flash
        # checkpoint ships). sglang calls this method for every per-image token
        # count, so a mismatch is a token-count bug, not a cosmetic one.
