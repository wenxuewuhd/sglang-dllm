...
class HashTopK(nn.Module):
    ...
    def forward(
        ...
        if _is_xpu:
            ...
            # CUDA/HIP kernel. NPU must stay on the device-native torch path
            # until an NPU fused hash routing kernel is wired in; otherwise the
            # first request attempts a CUDA JIT build and fails looking for
            # CUDA_HOME.
