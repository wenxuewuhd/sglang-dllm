...
Ported from the Ascend 910_93 single-card fork (Pan-Boyi/sglang@dsv4-cann9-no-patch).
The placement logic is unchanged; server-arg access is rewritten onto this tree's
``get_exec().moe`` / ``model_config_of()`` accessors.
"""
...
def build_logical_to_gpu_index(masks: torch.Tensor) -> torch.Tensor:
    ...
    # cumsum-1 over the mask is exactly "rank among the resident experts before
    # me", i.e. the packed slot; masked-out rows keep CPU_EXPERT_SLOT.
    slots = masks.long().cumsum(dim=1) - 1
    out[masks] = slots[masks]
    return out
...
def ensure_kt_layer_masks(server_args: "ServerArgs") -> None:
    ...
    experts_per_moe_layer = moe_args.kt_num_gpu_experts or 0
    strategy = moe_args.kt_expert_placement_strategy

    if strategy == "frequency":
        if not moe_args.kt_activation_freq_path:
            raise ValueError(
        ...
        activation_freq = load_activation_freq(moe_args.kt_activation_freq_path)
        ...
        raise ValueError(
            "Unknown --kt-expert-placement-strategy "
            f"{strategy!r}; expected 'prefix' or 'frequency'."
        )
