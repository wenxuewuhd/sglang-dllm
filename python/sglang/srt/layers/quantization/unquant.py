...
if _is_npu:
    ...
    def _npu_moe_loader_wrapper(
        ...
        """Wrap original loader: call it, then only after last slice convert ND->NZ (avoid OOM, no 2x HBM).
        ...
        def wrapper(
            ...
            if n >= expected_slices:
                param.data = npu_format_cast(
                    param.data.transpose(1, 2).contiguous()
                )
...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase, MultiPlatformOp):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # NPU: convert ND -> NZ in process_weights (if converted during loading, no need to convert again).
        if _is_npu:
