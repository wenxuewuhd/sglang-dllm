...
# Test npu_fused_infer_attention_score_v2 for chunk prefill (chunk=4096, seq_len 遍历 2k/4k/8k/16k/32k) with:
...
class TestNpuFiaBlockwisePaGraphV2(unittest.TestCase):
    """
        ...
        for S in self.seq_lens_to_test:
            with self.subTest(seq_len=S):
                ...
                if S <= chunk_len:
                    ...
                    out_fia, _ = torch.ops.npu.npu_fused_infer_attention_score_v2(
                        query_tnd,
                        k_paged,
                        v_paged,
                    ...
                    for c in range(num_chunks):
                        ...
                        out_c, _ = torch.ops.npu.npu_fused_infer_attention_score_v2(
                            q_chunk,
                            k_paged,
                            v_paged,
                            block_table=block_table,
    ...
    def test_4_npu_graph_capture_replay_fia_pa_block_wise(self):
        ...
        S>chunk_len: 只捕获一次（单次 FIA），循环 replay，每次 replay 前 graph.update(actual_seq_qlen, actual_seq_kvlen) 并写入当前 chunk 的 query/mask（参考 npu_graph_runner replay 前 _update_inputs）。
        """
        N, KV_N, D = self.num_heads, self.num_kv_heads, self.head_dim
        chunk_len = self.chunk_len
        for S in self.seq_lens_to_test:
            with self.subTest(seq_len=S):
                if S > chunk_len:
                    ...
                    kv_len_max = S
                    actual_seq_qlen = [cur_len_max]
                    actual_seq_kvlen = [kv_len_max]
                    ...
                    def run_fia(q_in, k_p, v_p, mask, alen, alen_kv):
                        o, _ = torch.ops.npu.npu_fused_infer_attention_score_v2(
                            q_in,
                            k_p,
                            v_p,
                            block_table=block_table,
                            ...
                            atten_mask=mask,
                            softmax_scale=self.scale,
                            actual_seq_qlen=alen,
                            actual_seq_kvlen=alen_kv,
                            sparse_mode=0,
                        )
                        return o
                    ...
                    update_dict = {"actual_seq_qlen": [0], "actual_seq_kvlen": [0]}
                    ...
                    for c in range(num_chunks):
                        ...
                        # 安全更新标量参数（复用字典内存）
                        update_dict["actual_seq_qlen"][0] = cur_chunk_len
                        update_dict["actual_seq_kvlen"][0] = kv_len
    ...
    def test_5_fia_batched_tnd_pa_block_wise(self):
        ...
        for B in batch_sizes_to_test:
            with self.subTest(batch_size=B):
                ...
                actual_seq_qlen_max = [cur_len_max * (b + 1) for b in range(B)]
                actual_seq_kvlen_max = [kv_len_max for _ in range(B)]
                ...
                update_dict = {"actual_seq_qlen": [0] * B, "actual_seq_kvlen": [0] * B}
                ...
                for c in range(num_chunks):
                    ...
                    # 写入当前 batch 的正确长度参数
                    for b in range(B):
                        update_dict["actual_seq_qlen"][b] = cur_chunk_len * (b + 1)
                        update_dict["actual_seq_kvlen"][b] = kv_len
